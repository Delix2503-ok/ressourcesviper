...
AddEventHandler('QBCore:Server:OnPlayerUnload', function(source)
    ...
    if player then
        ...
        if inv then
            local blackMoney = Inventory.GetItemCount(inv, 'black_money')
            if blackMoney and blackMoney ~= player.Functions.GetMoney('blackmoney') then
                player.Functions.SetMoney('blackmoney', blackMoney, "Sync blackmoney on disconnect")
            end
        end
    end
...
-- Ne pas écraser le black_money si ox en a déjà un sauvegardé
local existingBlack = Inventory.GetItemCount(Inventory(Player.PlayerData.source), 'black_money')
if not existingBlack or existingBlack == 0 then
    Inventory.SetItem(Player.PlayerData.source, 'black_money', Player.PlayerData.money.blackmoney or 0)
end
...
function server.syncInventory(inv)
    ...
    if accounts then
        ...
        if accounts.black_money and accounts.black_money ~= player.Functions.GetMoney('blackmoney') then
            player.Functions.SetMoney('blackmoney', accounts.black_money, "Sync blackmoney with inventory")
        end
    end
end
    ...
    if type(items) == 'table' then
        ...
        if player then
            for name in pairs(server.accounts) do
                ...
                if not hasThis then
                    local amount = player.Functions.GetMoney(name == 'money' and 'cash' or name == 'black_money' and 'blackmoney' or name)
