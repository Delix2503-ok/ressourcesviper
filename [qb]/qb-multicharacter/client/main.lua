...
RegisterNetEvent('qb-multicharacter:client:spawnLastLocation', function(coords, cData)
    ...
    SetEntityCoords(ped, -1302.25, 265.1, 63.46)
    SetEntityHeading(ped, 183.25)
...
RegisterNUICallback('setupCharacters', function(_, cb)
    QBCore.Functions.TriggerCallback('qb-multicharacter:server:setupCharacters', function(result)
        cached_player_skins = {}
        if #result == 1 then
            -- Un seul perso : spawn direct sans afficher la sélection
            SendNUIMessage({ action = 'autoSpawn' })  -- cache l'UI avant que le timer JS la réaffiche
            DoScreenFadeOut(10)
            TriggerServerEvent('qb-multicharacter:server:loadUserData', result[1])
            SetNuiFocus(false, false)
            skyCam(false)
            SetEntityAsMissionEntity(charPed, true, true)
            DeleteEntity(charPed)
        else
            -- 0 perso ou plusieurs : envoyer au NUI (le JS redirige vers création si 0 perso)
            SendNUIMessage({
